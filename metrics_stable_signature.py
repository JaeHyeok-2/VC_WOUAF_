...
@torch.no_grad()
def get_bit_accs_memoriable(img_dir: str, msg_decoder: nn.Module, key: torch.Tensor, batch_size: int = 16):
    # resize crop
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ...
    for ii, imgs in enumerate(tqdm.tqdm(data_loader)):
        ...
        decoded = msg_decoder(imgs) # b c h w -> b k
        diff = (~torch.logical_xor(decoded>0, keys>0)) # b k -> b k
        bit_accs = torch.sum(diff, dim=-1) / diff.shape[-1] # b k -> b
        for jj in range(bit_accs.shape[0]):
            img_num = ii*batch_size+jj
            log_stat = log_stats[img_num]
            log_stat[f'bit_acc_memoriable'] = bit_accs[jj].item()
...
def main(params):
    ...
    if params.eval_imgs:
        ...
        numeric_df = img_df.select_dtypes(include=['number'])
        mean_row = numeric_df.mean()
        mean_row['Label'] = 'Mean'
        img_df.loc['Mean'] = mean_row
        ...
        lpips_mean, lpips_std, lpips_max, lpips_min = np.mean(lpipss), np.std(lpipss), np.max(lpipss), np.min(lpipss)
    ...
    if params.eval_vid:
        vid_metrics = get_video_metric(params.img_dir, params.img_dir_nw)
        print(f"fvd: {vid_metrics['fvd']:.4f}")
        print(f"tLP: {vid_metrics['tLP']:.4f}")
        print(f"tOF: {vid_metrics['tOF']:.4f}")
        
        log_path = os.path.join(params.output_dir + params.output_data_name, 'vid_metrics.txt')
        with open(log_path, "w") as file:
            for name, value in vid_metrics.items():
    ...
    if params.eval_bits_memoriable:
        ...
        key = torch.tensor([k=='1' for k in params.key_str]).to(device)
        log_stats = get_bit_accs_memoriable(params.img_dir_nw, msg_decoder, key, batch_size=params.batch_size)
        print(f'>>> Saving log stats to {params.output_dir}...')
        df = pd.DataFrame(log_stats)
        df.loc['Mean'] = df.mean()
        log_path = os.path.join(params.output_dir  + params.output_data_name, 'log_stats_memoriable.csv')
        os.makedirs(os.path.dirname(log_path), exist_ok=True)
        df.to_csv(log_path, index=False)
        print(df)
...
def get_parser():
    ...
    aa("--eval_bits", type=utils.bool_inst, default=False, help="")
    aa("--eval_bits_memoriable", type=utils.bool_inst, default=False, help="")
