...
def batch_ddim_sampling(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,\
    ...
    x_T = None
    batch_variants = []
    pattern = torch.tensor([1, 0])  # 기본 패턴
    wm_message = pattern.repeat(24) 
    ...
    for _ in range(n_samples):
        if ddim_sampler is not None:
            ...
            print("Samples' Shape을 확인 : ", samples.shape)
        ## reconstruct from latent to pixel space
        batch_images = model.decode_first_stage_2DAE(samples,wm_message)
